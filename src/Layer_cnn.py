from src.Layer_fc import *


class Conv_Layer(nn.Conv2d):
    def __init__(self, in_dims, opt, in_channels=1, out_channels=8, num_classes=10, kernel_size=7, stride=1, padding=1,
                 maxpool=False, act_fn="relu", batchnorm=False, groups=1, droprate=0, loss_criterion='CwC', ClassGroups=None):
        ...
        self.optim = Adam(self.parameters(), lr=self.lr)  # , weight_decay=1e-4
        self.threshold = 2.0
    ...
    def goodness_factorCW(self, y, gt):
        ...
        # print('gf', gf.shape)
        # print("goodness_factors", len(goodness_factors))
        # Shapes: y_sets: [num_classes, B, C/num_classes, H, W], gf: [B, num_classes]
        # print("self.num_classes", self.num_classes)
        # print("self.outc", self.outc)
        # print('gf shape', gf.shape)
        # print('y_sets shape', len(y_sets))
        ...
        g_pos = gf[pos_mask].view(-1, 1) # g^+ in the paper
        g_neg = gf[neg_mask].view(gf.shape[0], -1).mean(1).unsqueeze(-1) # g^- in the paper
    ...
    def forward_forward(self, x_pos, gt):
...
class Conv_Layer_transpose(nn.ConvTranspose2d):
    def __init__(self, in_dims, opt, in_channels=1, out_channels=8, num_classes=10, kernel_size=7, stride=1, padding=1, output_padding=0,
                 maxpool=False, act_fn="relu", batchnorm=False, groups=1, droprate=0, loss_criterion='CwC', ClassGroups=None):
        super(Conv_Layer_transpose, self).__init__(in_channels, out_channels, kernel_size=kernel_size, stride=stride,
    ...
    def forward(self, x):
        ...
        # Forward Pass
        x = super(Conv_Layer_transpose, self).forward(x)
        if self.act_fn is not None:
            x = self.act_fn(x)
        if self.ismaxpool:
            x = self.maxpool(x)
