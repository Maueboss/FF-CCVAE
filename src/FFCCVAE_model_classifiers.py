...
class FFCCVAE(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.show_iters = 800
        self.sf_pred = self.opt.FFCCVAE.sf_pred
        ...
        self.loss_AE = self.opt.FFCCVAE.loss
        ...
        self.C_max = torch.Tensor([self.opt.FFCCVAE.max_capacity])
        ...
        if self.opt.input.dataset == 'mnist':
            # Automatic setup for MNIST
            # self.enc_model = [30,60]
            # self.dec_model = [30,1]
            # self.latent_shape = self.opt.FFCCVAE.latent_shape
            # self.maxpool = False
            # self.n_groups = self.opt.FFCCVAE.n_groups
            ...
            # If Grayscale:
            # CNN_l1_dims = [1, 32, 32]  # RGB images, 32x32
        ...
        for i, out_channels in enumerate(self.enc_channel_list):
            if self.ClassGroups:
                ...
                self.kernel = self.opt.FFCCVAE.enc_kernel[i]
        ...
        for i, out_channels in enumerate(self.dec_channel_list):
            if self.ClassGroups:
                #ClassGroup case for CIFAR-100
                self.kernel = self.opt.FFCCVAE.dec_kernel[i]
        ...
        # First classifier try
        layers = []
        layers.append(nn.Linear(self.enc_channel_list[-1]*self.latent_shape[0]* self.latent_shape[1], self.n_classes))
        ...
        self.linear_classifier_1 = nn.Sequential(*layers)

        

        # Second classifier try
        ...
        self.linear_classifier_2 = nn.Sequential(*layers)

        # Third classifier try

        layers = []
        layers.append(nn.Linear(self.dec_channel_list[-1]*CNN_l1_dims[1]*CNN_l1_dims[2], self.n_classes))
        ...
        self.linear_classifier_3 = nn.Sequential(*layers)
    ...
    def _init_weights(self):
        ...
        for m in self.linear_classifier_1.modules():
            if isinstance(m, nn.Linear):
                nn.init.zeros_(m.weight)

        for m in self.linear_classifier_2.modules():
            if isinstance(m, nn.Linear):
                nn.init.zeros_(m.weight)
        
        for m in self.linear_classifier_3.modules():
            if isinstance(m, nn.Linear):
                nn.init.zeros_(m.weight)
    ...
    def _N_classes(self):
        if self.opt.input.dataset == "cifar10" or self.opt.input.dataset == "mnist" or self.opt.input.dataset == "senti" or self.opt.input.dataset == "fmnist":
            ...
            return len(self.opt.input.GTSRB_classes)
    ...
    def _decoder_CVAE_mirror(self, h_pos,  y=None, y_n=None, mu=None, log_var=None, enc_list=None, h_neg = None, scalar_outputs=None, epoch=None, mode = "train"):
        ...
        if mode == "train":
            ...
            h_pos = h_pos.view(-1, self.opt.FFCCVAE.enc_channel_list[-1], self.latent_shape[0], self.latent_shape[0])
            ...
            for i, convlayer in enumerate(self.dec_model):
                s, e = start_end[i]
                if epoch in list(range(s, e)) or epoch is None:
                    h_pos = convlayer.forward(h_pos)
                    # print("Shape after layer",i, "decoder", h_pos.shape)
                    # print("Shape after layer",i, "decoder", enc_list[j].shape)
                    rec_loss, kld_loss = self.elbo_loss(h_pos, enc_list[j], mu, log_var)
                    scalar_outputs["Loss"] += rec_loss + kld_loss* self.beta
                    h_pos.detach()
                else:
    ...
    def forward(self, inputs, labels, epoch):
        ...
        if epoch < time:
            ...
            # print("Encoder:", h_pos.shape)
            with torch.no_grad():
                mse_cnn = utils.get_mse_cwc(h_pos, self.n_classes)
                accuracy_cnn = utils.get_accuracy(self.opt, mse_cnn, labels)
                # print("Accuracy CNN:", accuracy_cnn)
                scalar_outputs["Accuracy cwc"]  = accuracy_cnn

            if len(h_pos) > 2:
                    output = h_pos.reshape(h_pos.size(0), -1)

            output = self.linear_classifier_1(output.detach())
            loss_lc_1 = self.classification_loss(output, labels)
            accuracy_1 = utils.get_accuracy(self.opt, output.data, labels)
            scalar_outputs["Loss"] += loss_lc_1
            scalar_outputs["Accuracy 1"] = accuracy_1
            ...
            with torch.no_grad():
                ...
                # print("LOSS 1:",scalar_outputs["Loss"])
            

            h_pos, h_neg, mu, log_var, scalar_outputs = self._latent_CVAE_mirror(h_pos,onehot_y, scalar_outputs, y, y_n, onehot_y_neg)
            
            # Second classifier
            output = self.linear_classifier_2(h_pos.detach())
            loss_lc_2 = self.classification_loss(output, labels)
            scalar_outputs["Loss"] += loss_lc_2
            scalar_outputs["Accuracy 2"] = utils.get_accuracy(self.opt, output.data, labels)
            h_pos.detach()
            ...
            output = self.linear_classifier_3(output.detach())
            loss_lc_3 = self.classification_loss(output, labels)
            scalar_outputs["Loss"] += loss_lc_3
            scalar_outputs["Accuracy 3"] = utils.get_accuracy(self.opt, output.data, labels)
    ...
    def predict(self, inputs, labels, visualize=False, scalar_outputs=None, conditioned_generation=True, num_generate=25, grid_size=0.05):
        if scalar_outputs is None:
            scalar_outputs = {
            ...
            # TRYYY
            with torch.no_grad():
                h_pos, onehot_y, activations, scalar_outputs = self._encoder_CVAE_mirror(z, labels, scalar_outputs, mode = "test")
                ...
                h_pos, mu, log_var, activations, scalar_outputs = self._latent_CVAE_mirror(h_pos, onehot_y, scalar_outputs, activations= activations, mode = "test")
                ...
                h_pos, scalar_outputs = self._decoder_CVAE_mirror(h_pos, scalar_outputs = scalar_outputs, mode="test")
            ...
            # Multi-pass classification task
            ssq_all = []
            accuracies_all = []
            for i in range(self.n_classes):
                ...
                with torch.no_grad():
                    ...
                    mse_cnn_squared = torch.sum(mse_cnn ** 2, dim=-1)
                    mse_cnn_squared_mean = mse_cnn_squared.mean()
                    mse_cnn_squared_std = mse_cnn_squared.std()
                    mse_cnn_squared = (mse_cnn_squared - mse_cnn_squared_mean) / mse_cnn_squared_std

                    accuracies_all.append(mse_cnn_squared)
                    
                    # print(len(accuracies_all))
                    # print(accuracies_all[0].shape)
                    
                    # print("Accuracy CNN:", accuracy_cnn)
                    
                    h_pos, mu, log_var, activations, scalar_outputs = self._latent_CVAE_mirror(h_pos, onehot_y, scalar_outputs, activations= activations, label = i, mode = "test")
                    ...
                    ssq_mean = ssq.mean()
                    ssq_std = ssq.std()
                    ssq = (ssq - ssq_mean) / ssq_std
                    ssq_all.append(ssq)
        
                    
                    
            ssq_all = torch.stack(ssq_all, dim=-1).squeeze(1)
            # print(ssq_all)
            # print(ssq_all.argmax(dim=1))
            # print(labels)
            
            accuracy_multi_pass = utils.get_accuracy(self.opt, ssq_all, labels)
            # print(labels, ssq_all)
            # print(accuracy)
            scalar_outputs["Accuracy_multi_pass"]  = accuracy_multi_pass

            # print(accuracies_all[0])
            # print(accuracies_all[1])
            # print(accuracies_all[2])
            # print(accuracies_all[3])
            # print(accuracies_all[4])
            # print(accuracies_all[5])
            # print(accuracies_all[6])
            # print(accuracies_all[7])
            # print(accuracies_all.shape)
            # accuracies_all = torch.stack(accuracies_all, dim=-1)
            # print(accuracies_all.shape)
            # accuracy_cwc = utils.get_accuracy(self.opt, accuracies_all, labels)
            # scalar_outputs["Accuracy_cwc"] = accuracy_cwc

            # print(len(accuracies_all))
            # print(accuracies_all[0].shape)
            accuracies_all = torch.stack(accuracies_all, dim=-1).squeeze(1)
            # print(accuracies_all)
            # print(labels)
            # print(accuracies_all.argmax(dim=1))
            # # print(accuracies_all.to(dtype=torch.int64).argmax(dim=-1))
