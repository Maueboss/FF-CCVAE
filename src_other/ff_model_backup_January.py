...
from src_other import utils, Layer_cnn, Layer_fc
...
class FF_model(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        elif self.opt.model.structure == "VAE":
            self.batch_size = self.opt.input.batch_size
            self.show_iters = 800
        ...
        if self.opt.model.structure == "VFFAE":
            self.n_classes = self._N_classes()
    ...
    def _init_weights(self):
        if self.opt.model.structure == "AE" or self.opt.model.structure == "VAE" or self.opt.model.structure == "VFFAE":
    ...
    def _decoder_CVAE(self, h_pos, h_neg, y, y_n, scalar_outputs=None, mode = "train"):
    ...
    def _forward_VAE(self, inputs, labels, scalar_outputs):
        ...
        h_pos, scalar_outputs = self._decoder_CVAE(h_pos, h_neg, y, y_n, scalar_outputs)
    ...
    def predict_AE(self, inputs, labels, visualize =False, scalar_outputs = None):
        ...
        if visualize:
            ...
            for i, convlayer in enumerate(self.dec_model):
                h_pos = convlayer.ff_infer(h_pos)  # Assuming each decoder layer has a custom forward method `ff_infer`
